---
title: "Resources from workshop at Webstock 2018"
date: 2018-02-08T22:25:28+01:00
draft: true
---

## Disabilities and impairments

* [One in four New Zealanders identified as disabled](https://www.stats.govt.nz/news/one-in-four-new-zealanders-identified-as-disabled)
* [New Zealand Office For Disability Issues](https://www.odi.govt.nz/nz-disability-strategy/)
* [Disability Etiquette](https://www.odi.govt.nz/about-disability/disability-etiquette/)
* [All Technology Is Assistive](https://www.wired.com/2014/10/all-technology-is-assistive/)
* [Accessibility according to actual people with disabilities](https://axesslab.com/accessibility-according-to-pwd/)
* [Reframing Accessibility for the Web](https://alistapart.com/article/reframing-accessibility-for-the-web)

## Copy and content

* [Five Steps to Plain Language](http://centerforplainlanguage.org/learning-training/five-steps-plain-language/)
* [Robot Hug’s guide to writing image description](https://twitter.com/RobotHugsComic/status/949324466257039360)
* [Accessible emoji](https://tink.uk/accessible-emoji/)
* [Accessible emoji, tweaked](http://adrianroselli.com/2016/12/accessible-emoji-tweaked.html)

## Accessible HTML

* [HTML on Mozilla Developer Network](https://developer.mozilla.org/en-US/docs/Web/HTML)
* [HTML Elements Reference](https://developer.mozilla.org/en-US/docs/Web/HTML/Element
* [Understanding Semantics](https://tink.uk/understanding-semantics/)
* [Accessibility Through Semantic HTML](https://24ways.org/2017/accessibility-through-semantic-html/)
* [Accessible Rich Internet Applications (WAI-ARIA) 1.1](ARIA https://www.w3.org/TR/wai-aria-1.1/)

## Assistive Technology

* [Quick Tip: Safari Keyboard Navigation](https://a11yproject.com/posts/safari-keyboard-navigation/)
* [Examples of ‘skip links’ on some bank sites](https://developer.paciellogroup.com/blog/2012/08/examples-of-skip-links-on-some-bank-sites/)
* [Narrator keyboard commands and touch gestures](https://support.microsoft.com/en-gb/help/22806)
* [NVDA User Guide](https://www.nvaccess.org/files/nvda/documentation/userGuide.html?#toc18)
* [VoiceOver Getting Started Guide](https://help.apple.com/voiceover/info/guide/10.12/)
* [Basic screen reader commands for accessibility testing](https://www.paciellogroup.com/blog/2015/01/basic-screen-reader-commands-for-accessibility-testing/)

## Accessible Design

* [Web Content Accessibility Guidelines (WCAG) Overview](https://www.w3.org/WAI/intro/wcag.php)
* [WCAG 2.0 Guidelines](https://www.w3.org/TR/WCAG20/#guidelines)
* [Paciello Group’s Inclusive Design Principles](http://inclusivedesignprinciples.org)
* [Color Accessibility Workflows](https://abookapart.com/products/color-accessibility-workflows)
* [How To Design A Better Mobile Checkout Process](https://www.smashingmagazine.com/2013/03/designing-a-better-mobile-checkout-process/)

## Accessible Interaction

* [Respectful Collection of Demographic Data](https://medium.com/shenomads/respectful-collection-of-demographic-data-56de9fcb80e2)
* [Designing Safer Web Animation For Motion Sensitivity](https://alistapart.com/article/designing-safer-web-animation-for-motion-sensitivity)
* [An Introduction to the Reduced Motion Media Query](https://css-tricks.com/introduction-reduced-motion-media-query/)
* [HTML Elements Forms Guide](https://developer.mozilla.org/en-US/docs/Web/HTML/Element#Forms)

## Tools for reviewing and testing

* [Pa11y](https://validator.w3.org)
* [Koa11y](https://open-indy.github.io/Koa11y/)
* [WebAIM’s WAVE](http://wave.webaim.org)
* [Color Oracle](http://colororacle.org)
* [Heydon’s Inclusive Design Checklist](https://github.com/Heydon/inclusive-design-checklist)* [